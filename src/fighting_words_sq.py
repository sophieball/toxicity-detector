...
class FightingWords(Transformer):
  ...
  def __init__(self, obj_type="utterance", text_func=None, cv=None, ngram_range=None, prior=0.1,
    ...
    if self.cv is None:
      logging.info("Initializing default CountVectorizer")
      if self.ngram_range is None:
        self.ngram_range = (1, 3)
      logging.info("with ngram_range {}...".format(self.ngram_range))
  ...
  def _bayes_compare_language(self, class1: List[CorpusComponent], class2: List[CorpusComponent]):
    ...
    logging.info("Vocab size is {}.".format(str(vocab_size)))
    ...
    logging.info("Comparing language...")
    ...
    for i in sorted_indices:
      self.return_list.append({
              ...
              "count in class1": count_matrix[0, i],
              "count in class2": count_matrix[1, i]
      })
  ...
  def fit(self, corpus: Corpus, class1_func: Callable[[CorpusComponent], bool],
    ...
    logging.info("class1_func returned {} valid corpus components. "
    ...
    logging.info("ngram zscores computed.")
    return self
  ...
  def get_word_counts(self):
    return pd.DataFrame(self.return_list)
  ...
  def plot_fighting_words(self, max_label_size=15, class1_name='class1', class2_name='class2', config=None):
    ...
    """
    config = {'top_k': 20, 'annot_method': 'top_k'}# if config is None else config
    ...
    ax.legend()
    ax.set_xscale('log')
    plt.savefig("log-odds_ratio.PNG")
